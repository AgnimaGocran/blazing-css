	...
	let root_rules = if has_multi_value_array {
		...
		let processed_segments: Vec<String> = segments
			...
			.map(|seg| {
				seg.split_once(':')
					.and_then(|(p, v)| {
						let arr = parse_array_value(v.trim())?;
						let first = arr.first()?;
						Some(format!("{}: {}", p.trim(), first))
					})
...
fn format_media_queries_block(hash: &str, segments: &[String], break_points: &[u32]) -> String {
	...
	for (i, bp) in break_points.iter().enumerate() {
		let media_condition = format!("@media (max-width: {}px)", bp);

		let media_segments: Vec<String> = segments
			.iter()
			.map(|seg| {
				seg.split_once(':')
					.and_then(|(p, v)| {
						let arr = parse_array_value(v.trim())?;
						let selected = arr.get(i).or_else(|| arr.last())?.to_owned();
						Some(format!("{}: {}", p.trim(), selected))
					})
					.unwrap_or_else(|| seg.to_owned())
			})
			.collect();
	...
	let fallback_segments: Vec<String> = segments
		.iter()
		.map(|seg| {
			seg.split_once(':')
				.and_then(|(p, v)| {
